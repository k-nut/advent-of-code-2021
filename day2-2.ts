  ...
  const finalPosition = movements.reduce(
    (position: Position, movement) => {
      switch (movement.direction) {
        case "down":
          return { ...position, aim: position.aim + movement.distance };
        case "up":
          return { ...position, aim: position.aim - movement.distance };
        case "forward":
          return {
            ...position,
            depth: position.depth + position.aim * movement.distance,
...
await part2("./data/2-sample-1.txt");
await part2("./data/2-1.txt");
